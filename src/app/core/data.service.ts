...
export class DataService {
  private readonly allSigns = signal<TrafficSign[]>([
    {
      ...
      status: 'Verified',
      imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/a/a2/Stop_sign_light_red.svg/512px-Stop_sign_light_red.svg.png',
    },
    {
      id: 's2',
      lat: 40.741895,
      ...
      status: 'Verified',
      imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/8/8b/US_warning_yellow_diamond.svg/480px-US_warning_yellow_diamond.svg.png',
    },
    {
      id: 's3',
      lat: 40.721786,
      ...
      status: 'Pending',
      imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/6/64/Information_icon4.svg/512px-Information_icon4.svg.png',
    },
  ]);
  ...
  readonly stats = computed(() => {
    ...
    const pending =
      this.allSubmissions().length + signs.filter((s) => s.status === 'Pending').length;
  ...
  voteOnSubmission(id: string, approve: boolean) {
    const updated = this.allSubmissions().map((s) =>
      s.id === id
        ? {
            ...s,
  ...
  addNewSign(sign: Omit<TrafficSign, 'id' | 'status'> & { status?: SignStatus }) {
    ...
    this.allSigns.set([
      ...this.allSigns(),
      { id: newId, status: sign.status ?? 'Pending', ...sign },
    ]);
  }
